...
\begin{gather*}
    ...
    \\
     ...
     \\
     BIAS=W(\lambda)X^T X\beta-\beta=[W(\lambda)-(X^T X)^{-1}]X^T X\beta
\end{gather*}
...
We then can write:
 ...
 = \sigma^2 W(\lambda)[[X^TX+\lambdaI_p](X^TX)^{-1}[X^TX+\lambdaI_p]^T-(X^TX)]W(\lambda)^T \\
...
and 
 ...
 =\sigma^2 \lambdaz^Tz+\sigma^2\lambda^2z^T(X^TX)^{-1}z>0
 \end{gather*}
