...
x <- getURL("https://raw.githubusercontent.com/AlbertiMarco/DATA-SCIENCE-ASSIGNMENTS/master/EU%20labels.csv")
...
```{r results='hide', message=FALSE, warning=FALSE}
mytheme <- theme(plot.title= element_text(face="bold",colour = "antiquewhite4",size = (16),hjust = 0.5)) #setting theme for plots
...
for (i in 1:5){
  ...
  graph<-autoplot(pr.out,variance_percentage=FALSE,loadings=TRUE,
           loadings.label=TRUE,loadings.label.repel=TRUE,loadings.colour="coral",loadings.label.size=3,loadings.label.colour="grey35", scale=0,colour="gold2")+ggtitle(paste("The first two PCs for",toString(index[i,2]),"pollutant"))+mytheme 
