...
EU_labels<-read.csv("EU labels.csv", header = FALSE ,sep=";") #Short labelling of countries
...
Given the data structure and the exercises a-c requests, we decided that the optimal approach would be looping over the chunks of data containing information for each pollutant, producing without repeting the code the outputs all in one step. First we create some variables that will be used in the loop: 
...
i=4
  ...
  graph<-autoplot(pr.out,variance_percentage=FALSE,loadings=TRUE,
           ...
           loadings.label.colour="grey35",loadings.label.vjust = 1.2, scale=0,
           colour="gold2") #+geom_text_repel()
  print(graph)
  pve= 100* (pr.out$sdev ^2)/ sum(pr.out$sdev ^2)  #screeplot
  scree<-plot(seq(from = 1, to = 28),pve, type ="o", ylab="PVE ", xlab=" Principal Component ",
       col =" blue")
  print(scree)
