...
```
Were we to disregard the statistical pitfalls of this exercise, we can interpret the results from this plot as follows. To ease our analysis, we will present as well some basic descriptive statistics of the first two principal components' loadings.
```{r}
...
By plotting the values of the first principal component for all the pollutants, we can immediately appreciate how the trends of all of them are extremly similar, increasing monotoniclaly and linearly. The fact that they are alike even in levels is porbably due to the fact that all the five datasets have been rescaled before running the PCA.
